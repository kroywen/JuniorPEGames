package com.thepegeekapps.juniorpegames.database;
...
public class DatabaseHelper extends SQLiteOpenHelper {
	...
	public static final String DATABASE_NAME = "junior_pe_games";
	...
	public void importCsvToDatabase(SQLiteDatabase db) {
		...
		if (files != null && files.length > 0) {
			for (int i=0; i<files.length; i++) {
				String categoryTitle = files[i].substring(0, files[i].length()-4);
				...
				try {
					...
					while ((data = reader.readNext()) != null) {
		            	...
		            	gameValues.put(FIELD_STUDENTS, data[1]);
		            	gameValues.put(FIELD_EQUIPMENT, data[2]);
		            	gameValues.put(FIELD_AREA, data[3]);
		            	gameValues.put(FIELD_INSTRUCTIONS, data[4]);
		            	gameValues.put(FIELD_VARIATION, data[5]);
